...
export async function GET() {
  ...
  const [order, orderVersion, message, question, review, chatSession, counts] = await Promise.all([
    ...
    prisma.chatSession.findFirst({ where: { awaitingReplyFrom: "STAFF", chatClosedAt: null }, orderBy: { updatedAt: "desc" }, select: { id: true, name: true, email: true, updatedAt: true } }),
    Promise.all([
  ...
  return NextResponse.json({
    latest: {
      ...
      chatbot: chatSession ? { id: chatSession.id, product: chatSession.name ?? chatSession.email, at: chatSession.updatedAt } : null,
    },
