      ...
      <div className="mt-7 grid gap-6 lg:grid-cols-[1fr_300px]">
        <div className="space-y-6">
          <section className="rounded-xl2 border border-line bg-paper p-5 shadow-card sm:p-6">
            ...
            <ProfileForm name={account.name ?? ""} email={account.email} cpf={account.cpf} phone={account.phone} allowEmailUpdates={account.allowEmailUpdates} allowWhatsappUpdates={account.allowWhatsappUpdates} hasPassword={hasPassword} />
          </section>
        ...
        <aside className="space-y-4">
          <div className="rounded-xl2 border border-line bg-gradient-to-br from-brand-soft to-paper p-5 shadow-card">
            ...
            <p className="mt-4 font-semibold">{account.name || "Cliente Heca Store"}</p><p className="mt-0.5 break-all text-sm text-ink-muted">{account.email}</p>
